...
extern "C" {
	...
	static PyObject *py_glucose3_bumpvars  (PyObject *, PyObject *);
...
static PyMethodDef module_methods[] = {
	...
	{ "glucose3_bumpvars",  py_glucose3_bumpvars,  METH_VARARGS,    "Bump activity of important variables" },
...
//
//=============================================================================
static PyObject *py_glucose3_bumpvars(PyObject *self, PyObject *args)
{
	PyObject *s_obj;
	PyObject *v_obj;  // variables to bump given as a list of variable IDs

	if (!PyArg_ParseTuple(args, "OO", &s_obj, &v_obj))
		return NULL;
	...
	if (pyiter_to_vector(v_obj, v, max_var) == false)
		return NULL;

	if (max_var > 0)
		glucose3_declare_vars(s, max_var);

	// Bump activity for each variable (use absolute value in case literals are passed)
	for (size_t i = 0; i < v.size(); ++i) {
		int var_id = abs(v[i]);
		if (var_id > 0 && var_id <= s->nVars()) {
			s->varBumpActivity(var_id - 1);  // Convert to 0-indexed
		}
	}
