...
class Glucose3(object):
    ...
    def solve_with_suggestions(self, suggestions=[]):
        """
            Solve with soft constraints (suggestions).
            
            Unlike hard assumptions, suggestions guide the solver's search
            but can be violated if necessary to find a satisfying assignment.
            
            The solver will prefer to satisfy these suggested literals but
            will backtrack and ignore them if they lead to conflicts.
            
            Args:
                suggestions: List of integer literals (variable ID with sign)
                             Positive: suggest variable = TRUE
                             Negative: suggest variable = FALSE
        ...
        if self.glucose:
            ...
            # Set phase preferences based on suggestions
            # This guides the solver but doesn't enforce hard constraints
            if suggestions:
                self.set_phases(suggestions)
            
            # Solve without assumptions (suggestions are just phase hints)
            self.status = pysolvers.glucose3_solve(self.glucose, [],
                    int(MainThread.check()))
    ...
    def bump_var_activity(self, variables=[]):
        """
            Bump (increase) variable activity for given variables.
            
            This makes the solver prioritize these variables in decision making.
            Higher activity variables are chosen earlier in the search.
        ...
        if self.glucose:
            pysolvers.glucose3_bumpvars(self.glucose, variables)
